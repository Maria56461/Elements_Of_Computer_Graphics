...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(1.f, 0.5f, 0.f));
    }
...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color,
    const float kD, const float kS, const int shininess) {
    ...
    int lightPosLoc = glGetUniformLocation(shader->program, "light_position");
    glUniform3f(lightPosLoc, lightPosition.x, lightPosition.y, lightPosition.z);
    // trimit shader-ului valorile coordonatelor pozitiei sursei de lumina 
    ...
    int kD_Loc = glGetUniformLocation(shader->program, "material_kd");
    glUniform1f(kD_Loc, kD);

    int kS_Loc = glGetUniformLocation(shader->program, "material_ks");
    glUniform1f(kS_Loc, kS);

    int shininessLoc  = glGetUniformLocation(shader->program, "material_shininess");
    glUniform1i(shininessLoc, shininess);
